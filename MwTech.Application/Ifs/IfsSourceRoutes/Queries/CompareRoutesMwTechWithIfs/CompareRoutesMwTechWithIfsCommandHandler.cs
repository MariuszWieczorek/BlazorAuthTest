...
public class CompareRoutesMwTechWithIfsCommandHandler : IRequestHandler<CompareRoutesMwTechWithIfsCommand, CompareRoutesMwTechWithIfsViewModel>
{
    private readonly IApplicationDbContext _context;
    private readonly IOracleDbContext _oracle;
    private readonly IDateTimeService _dateTimeService;
    private readonly ICurrentUserService _currentUser;
    private readonly ILogger<CompareRoutesMwTechWithIfsCommandHandler> _logger;
    private readonly IProductCsvService _productCsvService;

    public CompareRoutesMwTechWithIfsCommandHandler(IApplicationDbContext context,
        IOracleDbContext oracle,
        IDateTimeService dateTimeService,
        ICurrentUserService currentUser,
        ILogger<CompareRoutesMwTechWithIfsCommandHandler> logger,
    ...
    public async Task<CompareRoutesMwTechWithIfsViewModel> Handle(CompareRoutesMwTechWithIfsCommand request, CancellationToken cancellationToken)
    {
        ...
        if (idxNo == 0)
        {
            additionalCondition = "AND ca.RouteSource = 0 AND ca.CategoryNumber not in ('DMA','DWA')";
        }

        if (idxNo == 2)
        {
            mwtechRoutesView = "dbo.MWTech_route_IFS_idx02";
           // additionalCondition = "AND MWT.ROUTING_REVISION >= 5";
        ...
        var comparedRoutes = _context.ComparedRoutesIfsVsMwTech
           .FromSqlRaw($@"
            SELECT *
            FROM (
            SELECT 
            ...
            ,MWT.Idx as Idx
            ,{idxNo} as IdxNo
            ,MWT.PART_NO as PartNo
            ,CAST(MWT.ROUTING_REVISION AS VARCHAR(10)) as RevisionNo
            ,CAST(MWT.ALTERNATIVE_NO AS VARCHAR(10)) as AlternativeNo
            ,CAST(MWT.OPERATION_NO AS VARCHAR(10)) as OperationNo
            --
            ...
            ,MWT.RoutePositionId
            --
            ,IIF(IFS.OperationNo IS NULL,0,1) as TestLineExists
            --
            ...
            ,IIF( ISNULL(TRIM(IFS.ToolId),'') = ISNULL(TRIM(MWT.TOOL_ID),'') ,1,0) as ToolIdTest
            ,IIF( ISNULL(IFS.ToolQuantity,0) = ISNULL(MWT.TOOL_QUANTITY,0) ,1,0) as ToolQuantityTest
            --
            FROM {mwtechRoutesView} as MWT
            --
            left join dbo.Products as pr
            on pr.Id = MWT.ProductId
            left join dbo.ProductCategories as ca
            on ca.Id = pr.ProductCategoryId
            --
            left join ( SELECT * FROM dbo.IfsRoutes WHERE 1 = 1
			AND (ISNUMERIC(AlternativeNo ) = 1 OR AlternativeNo = '*')
			AND (ISNUMERIC(RevisionNo ) = 1 OR RevisionNo = '*')	
			) as IFS
            ...
            WHERE 1 = 1 
            {additionalCondition}
            AND pr.IsTest = 0 -- dodane 20240119
            AND pr.IsActive = 1
        ...
        var compareRoutesMwTechWithIfsViewModel = new CompareRoutesMwTechWithIfsViewModel
        {
        ...
        return compareRoutesMwTechWithIfsViewModel;
    }
    ...
    public IQueryable<ComparedRouteIfsVsMwTech> Filter(IQueryable<ComparedRouteIfsVsMwTech> comparedRoutes, CompareRouteFilter filter)
    {
        if (filter != null)
        {
            ...
            if (filter.ProductNumber != null)
                comparedRoutes = comparedRoutes
                    .Where(x => x.PartNo.Contains(filter.ProductNumber));
        }
