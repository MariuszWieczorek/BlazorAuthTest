...
SELECT ca.CategoryNumber, cva.CategoryNumber
...
inner join dbo.ProductCategories as ca
on ca.Id = pr.ProductCategoryId
inner join dbo.ProductCategories as cva
on cva.Id = v.ProductCategoryId
inner join dbo.ManufactoringRoutes as r
on r.RouteVersionId = v.id
inner join dbo.Resources as wc
on wc.Id = r.WorkCenterId
...
AND ca.CategoryNumber = 'DMA'
AND v.IsActive = 1
GROUP BY ca.CategoryNumber, cva.CategoryNumber
--,wc.ResourceNumber,crew.ResourceNumber 
,r.ChangeOverLabourConsumption, r.ChangeOverMachineConsumption
,chcrew.ResourceNumber
,r.ChangeOverNumberOfEmployee
,r.MoveTime
,r.Overlap
ORDER BY ca.CategoryNumber, cva.CategoryNumber
