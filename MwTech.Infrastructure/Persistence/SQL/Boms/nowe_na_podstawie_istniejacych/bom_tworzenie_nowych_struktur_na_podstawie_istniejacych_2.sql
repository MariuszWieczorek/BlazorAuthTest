...
,CAST(REPLACE(spr.ProductNumber,@Symb1,@symb2) + @suffix as varchar(50)) as new_set
,CAST(REPLACE(REPLACE(spr.Name,@Name1,@Name2),@Symb1,@Symb2) + @suffix as varchar(150)) as new_set_name
,ppr.ProductNumber as ppart
,CAST(REPLACE(ppr.ProductNumber,@Symb1,@Symb2) + IIF(pca.CategoryNumber in ( 'DWY', 'DOB'), @suffix,'') as varchar(50)) as newpart
,CAST(REPLACE(REPLACE(ppr.Name,@Name1,@Name2),@Symb1,@Symb2) + IIF(pca.CategoryNumber in ( 'DWY', 'DOB'), @suffix,'') as varchar(150)) as new_part_name
,b.PartQty
,b.OrdinalNumber
,b.OnProductionOrder
,b.Excess
...
where 1=1 
and sv.IsActive = 1
and sca.CategoryNumber in ('ZAW-M', 'DWY', 'DOB','DAP','DST','DWU','DKJ','DET','DET-LUZ','DET-KBK','DWU-B','DKJ-B','DET-B')
...
on pca.Id = ppr.ProductCategoryId
where 1=1 
and sca.CategoryNumber in ('DWY', 'DOB','DAP','DST','DWU','DKJ','DET','DET-LUZ','DET-KBK','DWU-B','DKJ-B','DET-B')
...
, isnull(x.SetOldProductNumber,'') as SetOldProductNumber
, isnull(x.set_idx01,'') as matka
, isnull(x.set_idx02,'') as waz
...
/*
select n.ProductNumber
, n.ProductName
, n.Unit
, n.Categorty
, n.SetOldProductNumber
, n.matka
, n.waz
from ProductsToAdd as n
where n.ProductNumber not in ( select productNumber from dbo.Products)
...
,replace(x.PartQty, '.', ',') as PartQty
,replace(x.Excess, '.', ',') as Excess
,x.OnProductionOrder
,'A' as Method
,0 as layer
from AllBomsToAdd as x
where x.new_set not in ( select productNumber from dbo.Products)
group by x.new_set
,x.VersionName
,x.AlternativeNo
,x.VersionNumber
...
--
Where rv.IsActive = 1	
And a.ProductNumber not in ( select productNumber from dbo.Products)
