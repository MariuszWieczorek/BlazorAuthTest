...
declare @matka as varchar(20) = '10075153TR13';
/*
...
SELECT pr.idx01,ifs.ToolId
  ...
  group by ifs.ToolId ,pr.Idx01
*/


-- select ca.CategoryNumber, pr.ProductNumber, r.RoutingToolId, r.ToolQuantity, i.ToolId, r.id as PositionId, (select Id from dbo.RoutingTools where ToolNumber = i.toolid) as tool
...
inner join 
(
SELECT pr.idx01,ifs.ToolId, ifs.ToolQuantity, ifs.WorkCenterNo
  FROM [MwTech].[dbo].[IfsRoutes] as ifs
  inner join dbo.Products as pr
  on ifs.PartNo = pr.ProductNumber
  inner join dbo.ProductCategories as ca
  ...
  and pr.IsActive = 1
  and ca.RouteSource = 1
  group by ifs.ToolId ,pr.Idx01, ifs.ToolQuantity, ifs.WorkCenterNo
) as i
on i.Idx01 = pr.ProductNumber
where 1 = 1
AND v.ProductCategoryId = (select Id from dbo.ProductCategories where CategoryNumber = 'DWU')
