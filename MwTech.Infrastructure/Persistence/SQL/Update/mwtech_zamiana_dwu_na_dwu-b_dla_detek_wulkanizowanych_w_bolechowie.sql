...
from dbo.Products as p
  ...
  where ca.CategoryNumber IN ('DWU','DKJ','DET','DET-KBK')
  and p.Id in 
  (
  select w.ProductId
 ...
 update p
 set ProductCategoryId = (select Id from dbo.ProductCategories where CategoryNumber = 'DWU-B' )
from dbo.Products as p
  inner join dbo.ProductCategories as ca
  on ca.Id = p.ProductCategoryId
  where ca.CategoryNumber IN ('DWU')
  and p.Id in 
  (
  select w.ProductId
  from dbo.ManufactoringRoutes as r
